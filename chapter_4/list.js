...
// if I create two new values {value: 0, rest: list} and {value: -1, rest: list} (with list referring to the 
//     binding defined earlier), they are both independent lists, but they share the structure that makes up 
...
// argument. Also write a listToArray function that produces an array from a list. Then add a helper function prepend,
//  which takes an element and a list and creates a new list that adds the element to the front of the input list, and
//   nth, which takes a list and a number and returns the element at the given position in the list (with zero referring 
...
function nth(func, num) {
  for ( let node = func; node; node = node.rest ) {
    let temp = listToArray(node);
    return temp[num];
  }
}